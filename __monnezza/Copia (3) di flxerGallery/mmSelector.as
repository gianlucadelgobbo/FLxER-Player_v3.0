class flxerGallery.mmSelector extends MovieClip {
	...
	function thumbDrawer(w, h) {
		...
		var a;
		if (!noImg && this._visible) {
			trace("AAAAAAAAAAA"+noImg+" "+this._visible)
			this._parent["myToolbar"].piede.ss._visible = true;
		} else{
		...
		for (a=0; a<thumbNx*thumbNy; a++) {
			...
			if (this["puls"+a]) {
				...
				_root.myClassedMC(flxerGallery.buttonSelector, this, "puls"+a, {x:myX, y:myY, w:tw, h:th, fnzTrgt:this, fnz:"myOnPress", fnzOver:"myOnOver", param:a, src:lista.childNodes[a+thumbPage].childNodes[0].childNodes[0].attributes.src, tipo:tipo});
			}
			contaX++;
			if (contaX == thumbNx) {
				contaX = 0;
	...
	function myOnPress(a) {
		...
		trace("myOnPress "+this._parent["myToolbar"].piede.contr.playpause)
