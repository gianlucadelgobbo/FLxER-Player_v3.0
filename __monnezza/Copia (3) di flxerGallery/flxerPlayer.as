...
class flxerGallery.flxerPlayer extends MovieClip {
	...
	function flxerPlayer() {
		...
		//this._y = 0;
		h = h-(this._parent["myToolbar"].testa._height+this._parent["myToolbar"].piede.piedeEst._height+2);
		//h = h-(this._parent["myToolbar"].piede.piedeEst._height+1);
		trace("Altezza Schermo: "+h)
		trace("Larghezza Schermo: "+w);
		if (col != undefined) {
			_root.myDrawerFunc.drawer(this, "fondo", 0, 0, w, h, 0x6D6D6D, null, 100);
	...
	function setPos(ww, hh) {
		w = ww;
		h = hh-(this._parent["myToolbar"].testa._height+this._parent["myToolbar"].piede.piedeEst._height+2);
		if (col != undefined) {
			this["fondo"]._width = w;
	...
	function avvia(myXml, i, ss):Void {
		...
		ns.onStatus = function(infoObject) {
			...
			if (infoObject.code == "NetStream.Buffer.Full" && owner.firstTime2) {
				...
				owner._parent["myToolbar"].lab_i.htmlText = owner._parent["myToolbar"].tit;
			}
		};
		//ns.setBufferTime(5);
		/* new Sound */
	...
	function load_foto() {
		...
		if (col != undefined) {
			_root.myDrawerFunc.drawer(this["monitor"]["foto_"+l], "fondo", 0, 0, w, h, 0xDCE1E7, null, 100);
			//_root.myDrawerFunc.drawer(this["monitor"]["foto_"+l], "fondo", 0, 0, w, h, 0x6D6D6D, null, 100);
	...
	function avvia_flv() {
		...
		trace("DDDDDDDD "+this["monitor"].video.vid_flv._visible);
	...
	function MySetVolume() {
		trace(-(this._parent["myToolbar"].piede.contr.volume_ctrl.slider._y+10+this._parent["myToolbar"].piede.contr.volume_ctrl.slider._height))
	...
	function resizza() {
		...
		if (resizza_onoff) {
			if (tipo == "flv") {
				...
				trgt._width = w;
				trgt._height = h;
				if ((trgt._xscale/trgt._yscale)>(w/h)) {
					trgt._xscale = trgt._yscale;
				} else {
					trgt._yscale = trgt._xscale;
				}
