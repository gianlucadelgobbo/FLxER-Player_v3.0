	...
	function avvia(myXml, i):Void {
		...
		ns.onStatus = function(infoObject) {
			...
			if (infoObject.code == "NetStream.Buffer.Full") {
				...
				if (owner.autostop) {
					owner.autostop = false;
					this.close();
					owner.stop_play_flv();
					owner.nsclosed = true;
					if (owner._parent["myToolbar"].single) {
						owner._parent["myToolbar"]["ppBig"]._visible = true;
					}
					trace("GGGGGGGGGGGGGGGGGGG"+owner._parent["myToolbar"].single);
				}
