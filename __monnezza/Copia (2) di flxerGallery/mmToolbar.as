class flxerGallery.mmToolbar extends MovieClip {
	...
	function setPos(w, h) {
		...
		if (single) {
			...
			piede.contr.selector._visible = piede.toppa._visible=true;
		}
	...
	function apriEmbed() {
		...
		if (this._parent.startUrl.indexOf("http://") != -1) {
			var tmp = _root.mySitePath+_root.embePath+this._parent.startUrl;
		} else {
			var tmp = _root.mySitePath+_root.embePath+_root.mySitePath+this._parent.startUrl;
		}
		trgt["myEmbed"].lab_i.text = "<object type=\"application/x-shockwave-flash\" data=\""+tmp+"\" width=\""+_root.w+"\" height=\""+_root.w+"\">\n	<param name=\"movie\" value=\""+tmp+"\" />\n</object>";
	...
	function avviaSelector() {
		...
		trace("CCCCCCCCC "+this._parent["mySelector"].noImg);
		if (!trgt.noImg) {
